...
Page({
      ...
      utils.buyTrolley({
        eventDetail: {
          items: getApp().globalData.items.reduce((accumulator, group) =>
            accumulator.concat(group.items.map(item => ({ ...item, itemPro: '' }))), [])
        }
      })
      wx.navigateTo({
  ...
  radioClick(e) {
    ...
    this.setData({
      trolley,
      checkAll: this.selectedRadio.length === trolley.filter(item => item.putShelvesFlg || !item.inventoryCount === 0).length,
    })
